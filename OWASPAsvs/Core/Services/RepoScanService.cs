...
using OWASPAsvs.Infrastructure.Parsers;
...
public class RepoScanService
{
    ...
    public async Task<RepoScan> ScanAsync(string repoUrl, string userId, string? githubToken = null)
    {
        ...
        return (await _repo.GetByIdAsync(scan.Id))!;
    }
        ...
        List<Exigence> batch)
    {
        ...
        var raw = await _ai.CompleteAsync(_modelScan, system, user, 2500);
    ...
    private static List<ScanFinding> ParseFindings(int scanId, List<Exigence> batch, string raw)
    {
        ...
        try
        {
            ...
            var dtos = JsonSerializer.Deserialize<List<RepoFindingDto>>(json, opts);
        ...
        {
            // JSON parse failure → return empty; caller marks remaining as Present
        }
